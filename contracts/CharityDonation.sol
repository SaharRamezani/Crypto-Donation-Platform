...
contract CharityDonation {
    ...
    constructor() {
        ...
        _addCharity("Red Cross", "Humanitarian aid and disaster relief worldwide", payable(0x1111111111111111111111111111111111111111));
        _addCharity("UNICEF", "Children's rights and development advocacy", payable(0x2222222222222222222222222222222222222222));
        _addCharity("Doctors Without Borders", "Medical care in crisis zones", payable(0x3333333333333333333333333333333333333333));
        _addCharity("World Wildlife Fund", "Conservation and environmental protection", payable(0x4444444444444444444444444444444444444444));
        _addCharity("Save the Children", "Child welfare and education support", payable(0x5555555555555555555555555555555555555555));
    }
     ...
     */
    function donate(uint256 _charityId) external payable validCharity(_charityId) noReentrant {
        ...
        // Emit event
        emit DonationReceived(msg.sender, _charityId, msg.value, block.timestamp);
    }
     ...
     */
    function approveCharity(uint256 _proposalId) external onlyOwner {
        require(_proposalId > 0 && _proposalId <= proposalCount, "Invalid proposal ID");
        require(!proposals[_proposalId].isProcessed, "Proposal already processed");
        ...
        // Add as new charity
        _addCharity(proposal.name, proposal.description, proposal.walletAddress);
        
        emit CharityApproved(charityCount, proposal.name, proposal.walletAddress);
    }
     ...
     */
    function rejectCharity(uint256 _proposalId) external onlyOwner {
        require(_proposalId > 0 && _proposalId <= proposalCount, "Invalid proposal ID");
        require(!proposals[_proposalId].isProcessed, "Proposal already processed");
     ...
     */
    function withdrawFunds(uint256 _charityId) external noReentrant {
        require(_charityId > 0 && _charityId <= charityCount, "Invalid charity ID");
        Charity storage charity = charities[_charityId];
        require(msg.sender == charity.walletAddress, "Only charity wallet can withdraw");
     ...
     * @param _charityId The ID of the charity
     */
    function toggleCharityStatus(uint256 _charityId) external onlyOwner {
        require(_charityId > 0 && _charityId <= charityCount, "Invalid charity ID");
     ...
     */
    function getPendingProposals() external view returns (CharityProposal[] memory) {
     ...
     */
    function getDonorLeaderboard(uint256 _limit) external view returns (Donor[] memory) {
     ...
     */
    function getRecentDonations(uint256 _limit) external view returns (Donation[] memory) {
